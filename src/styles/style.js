const styles = {
  headingHero:
    "lg:text-[100px] text-[50px] text-gradient relative font-monoton z-20",
  paragraphHero:
    "text-[#FFB800] lg:text-[100px] text-[50px] uppercase select-none",
  skillsItem:
    "max-w-[200px] rounded-[58px] cursor-grab text-[#F5F5F5] hover:text-[#ffb800] hover:fill-[#ffb800] fill-[#f5f5f5] bg-gray-950 z-20 bg-opacity-85",
  heading2: "text-[64px] font-bold text-[#FFB800] uppercase select-none",
  outline:
    "hover:outline outline-[#F5F5F5] outline-offset-[0px] hover:outline-offset-[10px] hover:shadow-outline duration-100",
};

export default styles;
